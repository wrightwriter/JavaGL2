...
import _0_1.wrightgl.Pass.*
import _0_1.wrightgl.Pass.post.FXAA
...
class SketchDayBlabla(
    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            ...
            Engine.startSketch(SketchDayBlabla.javaClass, engineSettings)
        }
    }
    // ---------------- GUI ---------------- //
    ...
    val texture3DCompute: Texture = Texture(150,150,150)
...
//        val quadVB = VB( Geometry.quadTriangleStrip, intArrayOf(2), VB.PrimitiveType.TRIANGLES_STRIP )
    ...
    var raymarchFXFBCloud = PassRaymarchFB("raymarchCloud.fp" )

    var particlesCompute = PassCompute("particles.cp")
    var computePassTex = PassCompute("computeTex.cp")
...
//        thingQueue.add(cubeThing)
        ...
        raymarchFX.setUniformTexture("s_computeTex", texture3DCompute)
        raymarchFXFBCloud.setUniformTexture("s_computeTex", texture3DCompute)
        compositeFXFB.setUniformTexture("s_computeTex", texture3DCompute)

        compositeFXFB.setUniformTexture("s_raymarchedCloud", raymarchFXFBCloud.fb.textures[0])
...
//        particlesCompute.dispatch(particleThing.instancesBuffer)
        computePassTex.dispatch(texture3DCompute)
//        particleThing.render()


        raymarchFX.run(drawFB, true)
        raymarchFXFBCloud.run(drawFB, false)

        compositeFXFB.run(drawFB)
        fxaaPass.run(compositeFXFB.fb)

        postFX.run(fxaaPass.fb, defaultFB)
//        val mapFlags =
